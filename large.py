...
Electric Truck Charging Scheduling — Large-Scale Experiment
  ...
  No Exact Optimal
  Hard power constraint: P_STATION = 3500 kW
  Arrivals: uniform [06:00, 12:00) h, each truck independent
...
def rollout_scheduler(trucks, C, base_policy_fn, desc=""):
    ...
    if HAS_TQDM:
        iter_obj = tqdm(iter_obj, desc=desc if desc else "Rollout",
...
# ═══════════════════════════════════════════════════════
# MAIN EXPERIMENT  — loop over N_LIST
# ═══════════════════════════════════════════════════════
...
print(f"  C={C} | N_LIST={N_LIST} | K={K} | P_STATION={P_STATION} kW")
print(f"  Arrivals: [{ARRIVAL_LO:.0f}:00, {ARRIVAL_HI:.0f}:00)  "
      f"Deadline slack: {DEADLINE_SLACK}")
print("=" * 70)
...
for n_s in N_LIST:
    ...
    for k in range(K):
        ...
        for ro_key, bp_fn, ro_name in [
            ...
            _, J_ro, sched_ro = rollout_scheduler(
...
# ═══════════════════════════════════════════════════════
# SAVE CSV  — computation time per N per method
# ═══════════════════════════════════════════════════════
...
# Summary CSV — computation time per N (averaged over K)
...
# Summary table CSV — best rollout per N
...
with open(csv_table, 'w', newline='') as f:
    ...
    # Row 1: best rollout cost
    best_ro_costs = []
    best_ro_names = []
    best_ro_times = []
    base_costs    = []   # corresponding base policy (heuristic) cost

    for n_s in N_LIST:
        # find best rollout method for k=0
        ...
        # corresponding base policy cost (fcfs->fcfs, edf->edf, scdf->scdf)
        base_key = best_m.replace('ro_', '')   # 'ro_fcfs' -> 'fcfs'
        base_costs.append(results[n_s][base_key]['cost'][0])

    writer.writerow(['Best Rollout Cost (EUR)'] + [f'{c:.2f}' for c in best_ro_costs])

    # Row 2: cost reduction vs base policy (%)
    ...
    # Row 3: rollout computation time (s)
    writer.writerow(['Rollout Time (s)'] +
                    [f'{t:.2f}' for t in best_ro_times])

    # Row 4: which rollout method was best
    writer.writerow(['Best Rollout Method'] +
                    [m.upper().replace('_', '(').replace('RO(', 'RO(') + ')'
                     if ')' not in m else m for m in best_ro_names])
